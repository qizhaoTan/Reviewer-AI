...
func (p *AnthropicProvider) Generate(ctx context.Context, msgs []schema.Message, availableTools []schema.ToolDefinition) (*schema.Message, error) {
	system, anthropicMsgs := translateAnthropicMessage(msgs)

	anthropicTools, toolChoice := translateAnthropicTools(availableTools)
	...
	for _, block := range resp.Content {
		...
		case "tool_use":
			resultMsg.ToolCalls = append(resultMsg.ToolCalls, schema.ToolCall{
				ID:        block.ID,
				Name:      block.Name,
				Arguments: block.Input, // 响应里 Input 已是 json.RawMessage
			})
		}
	}
...
func translateAnthropicMessage(msgs []schema.Message) ([]anthropic.TextBlockParam, []anthropic.MessageParam) {
	system := make([]anthropic.TextBlockParam, 0, len(msgs))
			...
			for _, tc := range msg.ToolCalls {
				...
				if len(tc.Arguments) > 0 {
					if err := json.Unmarshal(tc.Arguments, &input); err != nil {
						panic(fmt.Errorf("反序列化历史 ToolCall 参数失败 (id=%s): %w", tc.ID, err))
					}
				}
...
// 翻译工具定义。InputSchema 是手写的 JSON Schema（map），直接灌进 ToolInputSchemaParam.Properties。
func translateAnthropicTools(tools []schema.ToolDefinition) ([]anthropic.ToolUnionParam, anthropic.ToolChoiceUnionParam) {
	...
	return anthropicTools, toolChoice
}
