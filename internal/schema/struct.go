...
type Message struct {
	Role    Role   `json:"role"`
	Content string `json:"content"` // 存放纯文本内容（已剥离思维链）

	// ReasoningContent 存放模型的思维链(<think>...</think> 或 reasoning_content)。
	// 仅用于日志/调试，不参与上下文回填，也不应直接对外发送。
	...
	// SenderID 是消息发送者的平台唯一标识（如飞书 open_id）。
	// 仅用于展示（Web 查看器把 user 消息渲染为真实姓名），不参与大模型上下文。
